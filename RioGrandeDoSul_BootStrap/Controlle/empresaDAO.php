<?php
    ...
    class empresaDAO{
        ...
        function deleteEmpresa($idUser){
            $pstmt = $this->conexao->prepare("DELETE FROM empresa WHERE userFKEmpresa = :idUser");
            $pstmt->bindValue(":idUser", $idUser);
            $pstmt->execute();
            return $pstmt;
        } 
        function updateEmpresa(empresa $empresa){
            $pstmt = $this->conexao->prepare("UPDATE empresa SET nome = ?, cnpj = ?, telefone = ?, 
            email = ?, pix = ?, prejuizo = ?, valorArrecadado = ?, endereco = ?, 
            cidade = ?, vagasDeEmprego = ?, empregadosTotal = ? 
            WHERE userFKEmpresa = ?");
            $pstmt->bindValue(1, $empresa->getTelefone());
            $pstmt->bindValue(2, $empresa->getCnpj());
            $pstmt->bindValue(3, $empresa->getTelefone());
            $pstmt->bindValue(4, $empresa->getEmail());
            $pstmt->bindValue(5, $empresa->getPix());
            $pstmt->bindValue(6, $empresa->getPrejuizo());
            $pstmt->bindValue(7, $empresa->getValorArrecadado());
            $pstmt->bindValue(8, $empresa->getEndereco());
            $pstmt->bindValue(9, $empresa->getCidade());
            $pstmt->bindValue(10, $empresa->getVagasDeEmprego());
            $pstmt->bindValue(11, $empresa->getEmpregadosTotal());
            $pstmt->bindValue(12, $empresa->getUserFKEmpresa());
            $pstmt->execute();
            return $pstmt;
        } 
    }
